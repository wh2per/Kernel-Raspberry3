...
static int simple_char_read(struct file *file, char *buf, size_t len, loff_t *lof){
	printk("simple_char read\n");
	return 0;
}

static int simple_char_write(struct file *file, const char *buf, size_t len, loff_t *lof){
	printk("simple_char_write: %s\n",buf);
	return 0;
}
